...
export function BrowserExtension({ onPairingCountChange }: { onPairingCountChange?: (count: number) => void }) {
  ...
  return <section className="extension-help" aria-labelledby="extension-title">
    ...
    <div className="extension-downloads">
      <a href="/extensions/zhiye-clipper-chrome.zip?v=0.3.3" download>下载 Chrome 扩展 0.3.3</a>
      <a href="/extensions/zhiye-clipper-firefox.zip?v=0.3.3" download>下载 Firefox 扩展 0.3.3</a>
    </div>
    ...
    <p>扩展只向 clip.sarainoq.cn 保存新副本；保存后会通知已打开的织页标签页刷新目录。图片只保留原链接，不上传登录凭证、完整 DOM 或登录态图片字节。</p>
