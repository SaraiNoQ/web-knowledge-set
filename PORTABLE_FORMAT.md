...
每个文档记录包含 `id`、`path`、`sha256`、`originalSha256`、标题、来源 URL、最终 URL、规范 URL、作者、发布时间、采集时间、标签、集合、收藏/归档状态、来源备注及 `assets`。可空元数据使用 `null`，集合字段使用数组。
...
- v1 内新增的可选字段可被旧读取器忽略；改变既有字段语义、路径规则或校验方式必须提升 `version`。
