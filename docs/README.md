...
> 当前仓库版本为 `0.9.2-rc.2` 功能完整预览版。它可提供 Apple Silicon 未签名 DMG，但未经 Apple Developer ID 签名或公证，也不配置自动更新；请勿把它当作正式发行版。

## 已有功能

- 公开网页采集、失败重试、队列暂停、采集历史和从已有快照重新提取。
- Markdown 编辑、预览、自动保存、修订历史和冲突保护；重新采集或 AI 结果不会静默覆盖人工正文。
- 标题、正文、来源的中英文搜索，以及一级文件夹、标签、多集合、收藏、归档、回收站、筛选和当前页批量操作。
...
3. 编辑 Markdown、标题、作者、发布日期和来源备注，用一级文件夹、标签与集合整理。
...
每篇知识最多归属一个一级文件夹；未选择时显示为“未分类”。文件夹与可多选的平面集合并存，删除文件夹只会把其中知识移回“未分类”，不会删除知识。
