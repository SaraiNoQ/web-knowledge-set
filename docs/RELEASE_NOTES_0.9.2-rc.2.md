...
`0.9.2-rc.2` 是功能完整预览版，用于在正式 `1.0.0` 之前验收主要功能、数据安全与发布流程。
...
- 修正 macOS 将 `/var` 规范化为 `/private/var` 时，发布测试故障注入路径无法匹配的问题。
...
1. 从本仓库的 `v0.9.2-rc.2` GitHub prerelease 下载 `Zhiye_0.9.2-rc.2_aarch64_unsigned.dmg` 和 `SHA256SUMS`，先核对 SHA-256。
